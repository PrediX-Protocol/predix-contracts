...
/// @title FeeExchangeHandler
/// @notice Invariant handler that drives randomized maker/taker activity with the FEE system ON: every order
///         carries a builder `code` (taker/maker bps) and the market has a protocol coef + rebate. Tracks
///         `totalMinted` so the suite can prove global USDC conservation (no wei created/destroyed/stranded)
///         across the maker-vs-maker + taker fee paths — the fuzz coverage the hand-written examples lack.
contract FeeExchangeHandler is Test {
    ...
    uint256 public totalMinted; // ghost: every wei of USDC ever minted into the system
    ...
    constructor(
        ...
        actors[0] = makeAddr("fh_alice");
        actors[1] = makeAddr("fh_bob");
        actors[2] = makeAddr("fh_carol");
        actors[3] = makeAddr("fh_dave");
        actors[4] = makeAddr("fh_eve");

        for (uint256 i; i < actors.length; ++i) {
            _mint(actors[i], 1_000_000e6);
    ...
    function placeBuy(uint256 actorSeed, uint256 priceSeed, uint256 amountSeed, bool yes) external {
        ...
        // deposit + headroom for the BUY fee prefund (makerFeeLocked + protocol reserve).
        _mint(a, (amount * price) / 1e6 + amount / 5);
        vm.prank(a);
        try exchange.placeOrder(
            marketId, yes ? IPrediXExchange.Side.BUY_YES : IPrediXExchange.Side.BUY_NO, price, amount, code
        ) returns (
            bytes32 id, uint256
        ) {
            orderIds.push(id);
    ...
    function placeSell(uint256 actorSeed, uint256 priceSeed, uint256 amountSeed, bool yes) external {
        ...
        vm.prank(a);
        try exchange.placeOrder(
            marketId, yes ? IPrediXExchange.Side.SELL_YES : IPrediXExchange.Side.SELL_NO, price, amount, code
        ) returns (
            bytes32 id, uint256
        ) {
            orderIds.push(id);
    ...
    function fill(uint256 actorSeed, uint256 sideSeed, uint256 amountSeed, uint256 limitSeed) external {
        ...
        vm.prank(a);
        try exchange.fillMarketOrder(marketId, side, limit, amountIn, a, a, 0, block.timestamp + 60, code) {} catch {}
    }

    function claimBuilder() external {
        try exchange.claimBuilderFee(code) {} catch {}
    }

    function sweepProtocol() external {
        try exchange.sweepProtocolFee() {} catch {}
    }
