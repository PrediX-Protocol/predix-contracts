...
///         real Arbitrum sequencer uptime feed. Unichain has no published
///         Chainlink feeds at the time of writing, so the integration is
///         verified on Arbitrum — the oracle adapter is chain-agnostic, so a
///         passing test here proves the same adapter wires correctly to
///         whichever chain eventually hosts a feed.
///
...
contract ChainlinkOracle_Arbitrum_Fork is Test {
    ...
    function setUp() public {
        ...
        diamond.setMarketWithEndTime(MARKET_ID, true, block.timestamp + 30 days);
    ...
    function test_Register_AgainstRealEthUsdFeed_Succeeds() public {
        uint64 snapshotAt = uint64(block.timestamp + 1 days);

        vm.expectEmit(true, true, false, true, address(oracle));
        emit IChainlinkOracle.MarketRegistered(
            MARKET_ID, ARB_ETH_USD_FEED, int256(1000e8), true, snapshotAt
        );
        ...
        assertTrue(cfg.gte);
        assertEq(cfg.snapshotAt, snapshotAt);
    }
