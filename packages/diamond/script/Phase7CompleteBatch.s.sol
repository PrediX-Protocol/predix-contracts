...
/// @notice Completion run for the 22-market batch after a partial broadcast.
/// @dev    Markets 117-123 already exist on Unichain mainnet (5 binary + event
///         27's two children) and are NOT recreated here. This script:
///           1. creates the 4 remaining events (-> child ids 124-138), and
///           2. seeds full-range ~100k USDC pools for ALL 22 markets (117-138).
///         Seed step is idempotent on hook register + pool initialize, so any
///         market that somehow already has a pool just gets liquidity appended.
///         Same two-signer model as Phase7CreateAndSeedBatch.
contract Phase7CompleteBatch is Script {
    ...
    function run() external {
        ...
        uint256[] memory ids = _idsAfterCreatingRemaining(e, creatorPk);
        console2.log("=========================================================");
        console2.log("All 22 marketIds (existing 117-123 + new):");
        ...
        console2.log("=========================================================");
        console2.log("COMPLETE: 4 remaining events created + full-range LP on all 22");
    ...
    function _idsAfterCreatingRemaining(Env memory e, uint256 creatorPk) internal returns (uint256[] memory ids) {
        uint256 t = block.timestamp;
        ids = new uint256[](22);
        uint256 p;

        // Already on-chain from the partial run — do NOT recreate.
        ids[p++] = 117;
        ids[p++] = 118;
        ids[p++] = 119;
        ids[p++] = 120;
        ...
        }
        {
            ...
            p = _event(
                ...
                ids,
                p,
                "Which major crypto posts the biggest 7-day return (window ending 2026-06-02)?",
                c,
                t + 345_600
            );
        }
        {
            ...
            p = _event(
                ...
                ids,
                p,
                "Who is the polling frontrunner for the 2028 Democratic nomination by 2026-06-03?",
                c,
                t + 475_200
            );
        }
