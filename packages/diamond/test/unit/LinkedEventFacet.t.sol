...
/// @notice Tasks 4/6/7: createLinkedEvent, mintCompleteSet/redeemCompleteSet, redeemLinked.
contract LinkedEventFacetTest is LinkedEventFixture {
    ...
    // ----------------------------- Task 4: createLinkedEvent -----------------------------

    function test_CreateLinkedEvent_SetsFlagsAndChildren() public {
        (uint256 eventId, uint256[] memory ids) = _createLinked3(endTime);

        assertTrue(linked.isLinkedEvent(eventId), "event not linked");
        assertEq(linked.eventPoolOf(eventId), 0, "fresh pool != 0");
    ...
    function test_MintCompleteSet_MintsOneYesPerOutcome() public {
        ...
        assertEq(linked.eventPoolOf(eventId), 10e6, "pool");
    ...
    function test_RedeemCompleteSet_ReversesMint() public {
        ...
        vm.startPrank(alice);
        linked.mintCompleteSet(eventId, 10e6);
        linked.redeemCompleteSet(eventId, 4e6);
        vm.stopPrank();

        assertEq(linked.eventPoolOf(eventId), 6e6, "pool after partial redeem");
    ...
    function test_Revert_MintCompleteSet_ZeroAmount() public {
        ...
        linked.mintCompleteSet(eventId, 0);
    }

    function test_Revert_MintCompleteSet_NotLinked() public {
        // legacy event id passed to a linked-only function
        string[] memory qs = _defaultQuestions(2);
        vm.prank(alice);
        (uint256 legacyId,) = eventFacet.createEvent("legacy", qs, endTime, address(eventOracle));
        vm.prank(alice);
        vm.expectRevert(ILinkedEventFacet.LinkedEvent_NotLinked.selector);
        linked.mintCompleteSet(legacyId, 1e6);
    }
    ...
    function test_RedeemLinked_PaysWinnerYesAndLoserNo() public {
        ...
        vm.startPrank(alice);
        linked.mintCompleteSet(eventId, 10e6); // y_i += 10 each
        market.splitPosition(ids[1], 5e6); // y_1 += 5, n_1 += 5
        vm.stopPrank();

        // pool = 10 (completeSet) + 5 (split) = 15
        assertEq(linked.eventPoolOf(eventId), 15e6, "pool pre-resolve");
        ...
        assertEq(linked.eventPoolOf(eventId), 0, "pool fully drained (no funds stuck)");
    ...
    function test_RedeemLinked_AnyWinner_PoolExactlySolvent() public {
        ...
        // drives strictly-increasing, always-future endTimes so each iteration's `createLinkedEvent`
        ...
        for (uint256 k; k < 3; ++k) {
            ...
            (uint256 eventId, uint256[] memory ids) = _createLinkedN(3, localEnd);
            _fundAndApprove(alice, 100e6);

            vm.startPrank(alice);
            linked.mintCompleteSet(eventId, 7e6);
            ...
            assertEq(linked.eventPoolOf(eventId), 0, "pool not drained");
        }
    }
    ...
    function test_Revert_RedeemLinked_NotResolved() public {
        (uint256 eventId,) = _createLinked3(endTime);
        vm.prank(alice);
        vm.expectRevert(ILinkedEventFacet.LinkedEvent_NotResolved.selector);
    ...
    function test_RedeemLinked_LinkedIsFeeFree_IgnoresDefault() public {
        ...
        _fundAndApprove(alice, 100e6);
        vm.startPrank(alice);
        linked.mintCompleteSet(eventId, 10e6); // pool 10, alice holds YES_0..2 = 10 each
        vm.stopPrank();
