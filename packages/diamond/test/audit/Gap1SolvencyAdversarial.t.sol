...
contract Gap1SolvencyAdversarial is LinkedEventFixture {
    ...
    function test_PoolIsolation_DrainingOneEventLeavesOtherExactlySolvent() public {
        ...
        (uint256 evA, uint256[] memory a) = _createLinkedN(3, endA);
        (uint256 evB, uint256[] memory b) = _createLinkedN(4, endB);
        ...
        vm.startPrank(alice);
        linked.mintCompleteSet(evA, 40e6);
        market.splitPosition(a[1], 10e6);
        vm.stopPrank();

        vm.startPrank(bob);
        linked.mintCompleteSet(evB, 25e6);
        market.splitPosition(b[2], 7e6);
        market.splitPosition(b[0], 3e6);
        vm.stopPrank();

        uint256 poolBbefore = linked.eventPoolOf(evB);
        ...
        vm.prank(alice);
        linked.redeemLinked(evA);

        assertEq(linked.eventPoolOf(evA), 0, "A must fully drain");

        // B must be COMPLETELY untouched and still exactly solvent for every winner.
        assertEq(linked.eventPoolOf(evB), poolBbefore, "B pool moved by another event's resolution");
    ...
    function testFuzz_DiamondCashAlwaysEqualsPool(uint256[16] calldata seed) public {
        ...
        (uint256 eventId, uint256[] memory ids) = _createLinkedN(3, endTime);
        ...
        for (uint256 i; i < seed.length; ++i) {
            ...
            if (op == 0) {
                ...
                vm.prank(u);
                linked.mintCompleteSet(eventId, raw);
            } else {
                ...
                vm.prank(u);
                linked.redeemCompleteSet(eventId, 1 + (raw % maxSet));
            }

            // CASH CONSERVATION: real money held == accounted pool. No fee pre-resolution.
            assertEq(usdc.balanceOf(address(diamond)), linked.eventPoolOf(eventId), "diamond cash diverged from pool");
            assertEq(market.totalCollateralLocked(), linked.eventPoolOf(eventId), "lockstep diverged from pool");
        }
        ...
        assertEq(linked.eventPoolOf(eventId), 0, "pool not drained to zero after full redemption");
    ...
    function test_EmergencyArbitraryWinner_StillExactlySolvent() public {
        ...
        (uint256 eventId, uint256[] memory ids) = _createLinkedN(4, endTime);
        ...
        vm.startPrank(alice);
        linked.mintCompleteSet(eventId, 50e6);
        ...
        uint256 bPay;
        vm.prank(alice);
        aPay = linked.redeemLinked(eventId);
        vm.prank(bob);
        bPay = linked.redeemLinked(eventId);
        ...
        assertEq(linked.eventPoolOf(eventId), 0, "pool not drained under arbitrary emergency winner");
    ...
    function test_DoubleRedeem_SecondCallReverts_PoolUnchanged() public {
        uint256 endTime = block.timestamp + 30 days;
        (uint256 eventId,) = _createLinkedN(3, endTime);
        _fundAndApprove(alice, 100e6);
        vm.prank(alice);
        linked.mintCompleteSet(eventId, 30e6);
        ...
        vm.prank(alice);
        uint256 first = linked.redeemLinked(eventId);
        ...
        assertEq(linked.eventPoolOf(eventId), 0, "pool moved on a failed second redeem");
    }
    ...
    function test_LoserOnlyHolder_CannotTouchPool() public {
        ...
        vm.prank(alice);
        linked.mintCompleteSet(eventId, 20e6);
        vm.prank(bob);
        ...
        uint256 poolBefore = linked.eventPoolOf(eventId);
        vm.prank(carol);
        vm.expectRevert(ILinkedEventFacet.LinkedEvent_NothingToRedeem.selector);
        linked.redeemLinked(eventId);
        assertEq(linked.eventPoolOf(eventId), poolBefore, "pool moved for a no-token caller");
    }
    ...
    function test_Finding_LinkedChildBypassesPerMarketCap() public {
        ...
        (uint256 eventId, uint256[] memory ids) = _createLinkedN(3, block.timestamp + 30 days);
        _fundAndApprove(alice, 1_000e6);
        vm.prank(alice);
        market.splitPosition(ids[0], 500e6); // 100x the cap — no revert

        assertEq(linked.eventPoolOf(eventId), 500e6, "linked child should have accepted the over-cap split");
    ...
    function test_MultiRedeemerMaxFee_PoolDrainsToZero_NoDust() public {
        ...
        vm.prank(alice);
        linked.mintCompleteSet(eventId, 10_000_001); // odd base unit
        vm.prank(bob);
        ...
        for (uint256 i; i < us.length; ++i) {
            vm.prank(us[i]);
            try linked.redeemLinked(eventId) returns (uint256 p) {
                paid += p;
            } catch {}
        }
        ...
        assertEq(linked.eventPoolOf(eventId), 0, "pool dust stranded under max fee + odd amounts");
