...
contract Audit_U2CAP_LinkedCapBypass is LinkedEventFixture {
    ...
    /// @dev The finding: a linked child pools BIG (>> CAP) with no cap check — via mintCompleteSet AND
    ...
    function test_U2CAP_LinkedChild_SilentlyBypassesPerMarketCap() public {
        ...
        (uint256 eventId, uint256[] memory ids) = _createLinked3(block.timestamp + 1 days);

        // mintCompleteSet pools BIG with no cap enforcement
        _fundAndApprove(bob, BIG);
        vm.prank(bob);
        linked.mintCompleteSet(eventId, BIG);
        assertEq(linked.eventPoolOf(eventId), BIG, "linked pool exceeded the per-market cap unchecked");

        // a single linked-child split also bypasses the cap (routes to the pool)
        _fundAndApprove(bob, BIG);
        vm.prank(bob);
        market.splitPosition(ids[0], BIG);
        assertEq(linked.eventPoolOf(eventId), BIG * 2, "second over-cap deposit also accepted");
    }
}
