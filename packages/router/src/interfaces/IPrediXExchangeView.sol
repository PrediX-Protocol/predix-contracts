...
///      required by the monorepo boundary rule — the router cannot import cross-package `src/`. This
...
interface IPrediXExchangeView {
    ...
    /// @dev `taker` parameter mirrors the self-match filter in `fillMarketOrder`
    ///      so preview output matches execute when caller has resting orders on
    ///      the opposite side. Pass `address(0)` to opt out of the filter.
